...
End-to-end tests that validate the `gentle-qa` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.ubuntu -t gentle-qa-e2e-ubuntu .
docker run --rm gentle-qa-e2e-ubuntu

# Run with full E2E on Arch
docker build -f e2e/Dockerfile.arch -t gentle-qa-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 gentle-qa-e2e-arch

# Interactive debugging
docker run --rm -it gentle-qa-e2e-ubuntu /bin/bash
```
...
4. Build the `gentle-qa` binary for `linux/amd64`
