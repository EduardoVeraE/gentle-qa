...
| Custom | `custom` | Keep your existing persona/config unmanaged — gentle-qa does not inject a persona |

`custom` is a compatibility/ownership choice, not a persona editor. Use it when you already have your own persona instructions and want gentle-qa to leave them alone.
...
If GitHub rate-limits update checks, export `GITHUB_TOKEN` or `GH_TOKEN` before running `gentle-qa update`/`upgrade`.
