...
This document explains how gentle-qa integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

gentle-qa supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, gentle-qa installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/gentle-qa.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running gentle-qa.
...
gentle-qa uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `gentle-qa install`.
...
This follows the same SDD architecture used in gentle-qa: orchestrator coordinates, phase agents execute, Engram persists artifacts across phases.
...
Kiro has a built-in spec workflow that gentle-qa leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
The steering file written by gentle-qa uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\gentle-qa.md` |
...
Kiro uses a **split-root layout** — gentle-qa managed files and IDE settings live in different directories:

- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/gentle-qa.md` — orchestrator persona
...
If gentle-qa skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
