...
Gentle-QA supports **two ways** of working with OpenCode profiles:

1. **Generated multi-profile mode** -- the classic Gentle-QA flow. The base SDD conductor is `gentle-orchestrator`. Each named profile generates its own `sdd-orchestrator-{name}` plus 10 suffixed sub-agents in `opencode.json`, and you switch between them with **Tab**.
...
That means you can stay with the built-in multi-profile overlay, or plug Gentle-QA into an external profile manager without the two systems fighting each other.
...
If you're using a community tool that stores profiles under `~/.config/opencode/profiles/*.json` and activates them at runtime, Gentle-QA can now sync OpenCode in a compatibility mode.

### Auto-detection

On `gentle-qa sync`, if OpenCode profile files exist under:

```text
~/.config/opencode/profiles/*.json
```

Gentle-QA automatically switches to **`external-single-active`** strategy for OpenCode sync.
...
```bash
gentle-qa sync --agent opencode --sdd-profile-strategy external-single-active
```

Or force the classic generated overlay behavior:

```bash
gentle-qa sync --agent opencode --sdd-profile-strategy generated-multi
```

### What compatibility mode does

In `external-single-active` mode, Gentle-QA:
...
This is the important bit: Gentle-QA still maintains the SDD foundation, but it stops acting like `opencode.json` is the source of truth for every profile.
...
During sync or update, Gentle-QA now uses one of two strategies:
