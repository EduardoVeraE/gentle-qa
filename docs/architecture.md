...
gentle-qa install --dry-run --agent claude-code --preset minimal

# Dry-run smoke test (Windows PowerShell)
gentle-qa.exe install --dry-run --agent claude-code --preset minimal
```
...
Install Gentleman.Dots first for your dev environment, then Gentle-QA for the AI layer on top.
