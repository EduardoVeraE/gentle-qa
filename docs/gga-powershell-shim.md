...
When `gentle-qa` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `gentle-qa install` is idempotent.
...
`gentle-qa install`**. In the final design this was moved to **runtime** — the `.ps1`
