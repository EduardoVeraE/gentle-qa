#!/usr/bin/env node
// mobile_artifacts.mjs — CLI to scaffold mobile testing artifacts from templates.
//
// Usage:
//   node mobile_artifacts.mjs list
//   node mobile_artifacts.mjs help [<template>]
//   node mobile_artifacts.mjs create <template> --out <dir> [--<placeholder> <value> ...] [--strip-hints]
//   node mobile_artifacts.mjs --self-test
//
// Make executable:
//   chmod +x mobile_artifacts.mjs
//
...
const DESCRIPTIONS = {
  "mobile-test-plan": "Mobile testing plan: platforms, OS coverage, tooling, schedule, exit criteria.",
  "device-matrix": "Device + OS coverage matrix with tiers and cloud-farm allocation.",
  "gesture-catalog": "Gesture interaction catalog with locator strategies and assertion patterns.",
  "mobile-bug-report": "Single mobile bug report with device context, repro steps, logs, environment.",
};
...
function generalHelp() {
  console.log(`mobile_artifacts — scaffold mobile testing artifacts from templates.
...
function cmdHelp(template) {
  ...
  console.log("");
  console.log(`Usage:`);
  console.log(`  node mobile_artifacts.mjs create ${template} --out ./out \\`);
...
function selfTest() {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "mobile-artifacts-"));
  try {
    for (const t of templates) {
