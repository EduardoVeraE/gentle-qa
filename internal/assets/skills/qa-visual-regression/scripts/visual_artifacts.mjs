#!/usr/bin/env node
// visual_artifacts.mjs — CLI to scaffold visual regression artifacts from templates.
//
// Usage:
//   node visual_artifacts.mjs list
//   node visual_artifacts.mjs help [<template>]
//   node visual_artifacts.mjs create <template> --out <dir> [--<placeholder> <value> ...] [--strip-hints]
//   node visual_artifacts.mjs --self-test
//
// Make executable:
//   chmod +x visual_artifacts.mjs
//
...
const DESCRIPTIONS = {
  "visual-test-plan": "Visual regression plan: tool choice, coverage targets, snapshot matrix, exit criteria.",
  "snapshot-matrix": "Browser × viewport × theme × locale × state matrix with snapshot budget.",
  "baseline-runbook": "Per-tool runbook for establishing, accepting, and rejecting visual baselines.",
  "visual-diff-report": "Triage report for a build's visual diffs: classification, root cause, action.",
};
...
function generalHelp() {
  console.log(`visual_artifacts — scaffold visual regression artifacts from templates.
...
function cmdHelp(template) {
  ...
  console.log("");
  console.log(`Usage:`);
  console.log(`  node visual_artifacts.mjs create ${template} --out ./out \\`);
...
function selfTest() {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "visual-artifacts-"));
  try {
    for (const t of templates) {
