#!/usr/bin/env node
// contract_artifacts.mjs — CLI to scaffold consumer-driven contract testing artifacts from templates.
//
// Usage:
//   node contract_artifacts.mjs list
//   node contract_artifacts.mjs help [<template>]
//   node contract_artifacts.mjs create <template> --out <dir> [--<placeholder> <value> ...] [--strip-hints]
//   node contract_artifacts.mjs --self-test
//
// Make executable:
//   chmod +x contract_artifacts.mjs
//
...
const DESCRIPTIONS = {
  "contract-test-charter": "Charter for a single consumer/provider Pact relationship: scope, state catalog, CI integration.",
  "broker-setup-checklist": "Pact Broker / PactFlow rollout checklist: environments, tokens, webhooks, retention.",
  "can-i-deploy-gate": "CI gate spec wiring can-i-deploy + record-deployment around a deploy step.",
  "pact-mismatch-report": "Triage report for a failed Pact verification: drift kind, root cause, fix, prevention.",
};
...
function generalHelp() {
  console.log(`contract_artifacts — scaffold consumer-driven contract testing artifacts from templates.
...
function cmdHelp(template) {
  ...
  console.log("");
  console.log(`Usage:`);
  console.log(`  node contract_artifacts.mjs create ${template} --out ./out \\`);
...
function selfTest() {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "contract-artifacts-"));
  try {
    for (const t of templates) {
