...
// TestRunArgs_UpgradeDryRun verifies that `gentle-qa upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `gentle-qa upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `gentle-qa upgrade engram` filters
...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	out := buf.String()
	// Output should only mention engram or no-upgrades, not gentle-qa or gga.
	// This is a soft check since the tool may not be installed.
	if strings.Contains(out, "gentle-qa") && !strings.Contains(out, "engram") {
		t.Errorf("filtering to engram should not show gentle-qa in output; got: %s", out)
	}
}
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"gentle-qa", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/EduardoVeraE/Gentle-QA/releases",
				},
			},
			wantContains:   []string{"gentle-qa", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "gentle-qa", "[--]", "gga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
