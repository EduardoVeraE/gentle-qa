...
// self-replace for gentle-qa is NOT attempted in Phase 1 — it must return a
...
func TestRunStrategy_BinaryWindowsSelfUpdateSkipped(t *testing.T) {
	...
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		ReleaseURL:    "https://github.com/EduardoVeraE/Gentle-QA/releases/tag/v1.5.0",
	}
...
func TestRunStrategyOpenCodePluginUpgradesMaterializedPackage(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		cmd.Env = append(os.Environ(),
			"GENTLE_QA_UPGRADE_HELPER=1",
			"GENTLE_QA_UPGRADE_HELPER_CWD_FILE="+cwdFile,
		)
		return cmd
	}
	...
	wantArgs := []string{"add", pkg + "@latest"}
...
func TestOpenCodePluginUpgradeHelperProcess(t *testing.T) {
	if os.Getenv("GENTLE_QA_UPGRADE_HELPER") != "1" {
		return
	}
	cwd, err := os.Getwd()
	if err != nil {
	...
	if err := os.WriteFile(os.Getenv("GENTLE_QA_UPGRADE_HELPER_CWD_FILE"), []byte(cwd), 0o644); err != nil {
		_, _ = os.Stderr.WriteString(err.Error())
		os.Exit(2)
	}
	os.Exit(0)
...
func TestManualFallbackHint(t *testing.T) {
	r := update.UpdateResult{
		...
		LatestVersion: "1.5.0",
		UpdateHint:    "See https://github.com/EduardoVeraE/Gentle-QA/releases",
	}
	profile := system.PlatformProfile{OS: "windows", PackageManager: "winget"}
...
func TestBrewUpgrade_RunsUpdateBeforeUpgrade(t *testing.T) {
	...
	err := brewUpgrade(context.Background(), "gentle-qa")
	if err != nil {
...
func TestBrewUpgrade_UpdateFailureIsNonFatal(t *testing.T) {
	...
	execCommand = func(name string, args ...string) *exec.Cmd {
		...
		return exec.Command("echo", "Upgraded gentle-qa")
	}
