...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "gentle-qa uses build var",
			tool:         ToolInfo{Name: "gentle-qa", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "gentle-qa dev build",
			tool:         ToolInfo{Name: "gentle-qa", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "gentle-qa-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "gentle-qa-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	assertResult(t, results[0], "gentle-qa", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// gentle-qa has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("gentle-qa status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("gentle-qa expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "gentle-qa macOS",
			tool:    ToolInfo{Name: "gentle-qa"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "brew upgrade gentle-qa",
		},
		{
			name:    "gentle-qa linux",
			tool:    ToolInfo{Name: "gentle-qa"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "curl -fsSL https://raw.githubusercontent.com/EduardoVeraE/Gentle-QA/main/scripts/install.sh | bash",
		},
		{
			name:    "gentle-qa windows",
			tool:    ToolInfo{Name: "gentle-qa"},
			profile: system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			want:    "irm https://raw.githubusercontent.com/EduardoVeraE/Gentle-QA/main/scripts/install.ps1 | iex",
		},
		{
		...
		},
		{
			...
			tool:    ToolInfo{Name: "engram"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "gentle-qa upgrade (downloads pre-built binary)",
		},
		{
			name:    "engram windows",
			tool:    ToolInfo{Name: "engram"},
...
func TestRegistryContents(t *testing.T) {
	...
	// gentle-qa must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("gentle-qa DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (gentle-qa) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just gentle-qa to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("gentle-qa dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes gentle-qa to be reported as DevBuild.
...
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return exec.Command("false") }
	Tools = []ToolInfo{Tools[0]} // gentle-qa only
	...
	r := results[0]
	if r.Tool.Name != "gentle-qa" {
		t.Fatalf("tool = %q, want gentle-qa", r.Tool.Name)
	}
...
// gentle-qa with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only gentle-qa and engram for this test
	...
	// gentle-qa should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("gentle-qa status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip gentle-qa to avoid dev-build behavior)
