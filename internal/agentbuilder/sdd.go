...
// Example: <!-- gentle-qa:custom-agent:my-skill -->
const markerFormat = "<!-- gentle-qa:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /gentle-qa:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /gentle-qa:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- gentle-qa:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- gentle-qa:custom-agent:"
	suffix := " -->"
