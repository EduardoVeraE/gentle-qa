...
// Write/Read. Both `gentle-qa install` (CLI in run.go) and the TUI app
// (internal/app/app.go) write this field after a successful install so that
// `gentle-qa sync` regenerates the persona the user actually selected — not a
// hard-coded default.
...
// TestWriteCreatesStateDir verifies that Write creates the .gentle-qa directory
...
func TestWriteStateFilePath(t *testing.T) {
	home := t.TempDir()
	got := Path(home)
	want := filepath.Join(home, ".gentle-qa", "state.json")
	if got != want {
