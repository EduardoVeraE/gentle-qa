...
.EXAMPLE
    ...
    # Or download and run:
    Invoke-WebRequest -Uri https://raw.githubusercontent.com/EduardoVeraE/Gentle-QA/main/scripts/install.ps1 -OutFile install.ps1
    .\install.ps1
...
function Get-LatestVersion {
    ...
    try {
        $response = Invoke-RestMethod -Uri $url -Headers @{ "User-Agent" = "gentle-qa-installer" }
    } catch {
...
function Install-ViaBinary {
    ...
    $tmpDir = Join-Path $env:TEMP "gentle-qa-install-$(Get-Random)"
