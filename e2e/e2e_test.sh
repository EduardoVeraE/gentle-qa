#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for gentle-qa installer
#
...
if [ -z "$BINARY" ]; then
    echo "ERROR: gentle-qa binary not found. Build it first."
    exit 1
fi
...
test_version_command() {
    ...
    if echo "$output" | grep -q "gentle-qa"; then
...
test_cc_engram_injection() {
    ...
    if $BINARY install --agent claude-code --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:engram-protocol" "CLAUDE.md has engram-protocol section marker"
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:sdd-orchestrator" "CLAUDE.md has SDD section marker"
...
test_cc_persona_gentleman() {
    ...
    if $BINARY install --agent claude-code --component persona --persona gentleman 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:persona" "CLAUDE.md has persona section marker"
...
test_cc_skills_full() {
    ...
    if $BINARY install --agent claude-code --component skills --preset full-gentleman --persona neutral 2>&1; then
        ...
        # Full preset = 25 files: 11 SDD (incl judgment-day) + 5 foundation + 4 QE +
        # 4 QA (owasp/mobile/visual/contract) + _shared/SKILL.md
...
test_oc_engram_injection() {
    ...
    if $BINARY install --agent opencode --component engram --persona neutral 2>&1; then
        ...
        assert_file_contains "$agents_md" 'gentle-qa:engram-protocol' "AGENTS.md has engram-protocol section"
...
test_full_preset_claude_code() {
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-gentleman --persona gentleman 2>&1; then
        ...
        assert_file_contains "$claude_md" "gentle-qa:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "gentle-qa:persona" "Has persona section"
...
test_full_preset_opencode() {
    ...
    if $BINARY install --agent opencode --component engram --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-gentleman --persona gentleman 2>&1; then
        ...
        assert_file_contains "$agents_md" "gentle-qa:engram-protocol" "AGENTS.md has engram protocol"
        ...
        assert_file_not_contains "$agents_md" "<!-- gentle-qa:sdd-orchestrator -->" "AGENTS.md has no SDD section marker (opencode uses json agent)"
...
test_minimal_preset_opencode_only_engram_no_persona() {
    ...
    if $BINARY install --agent opencode --preset minimal --persona neutral 2>&1; then
        ...
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "gentle-qa:persona" "No persona marker in minimal preset"
...
test_minimal_preset_claude_only_engram() {
    ...
    if $BINARY install --agent claude-code --preset minimal --persona neutral 2>&1; then
        ...
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:engram-protocol" "Engram protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "gentle-qa:sdd-orchestrator" "Claude has SDD"
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    assert_file_contains "$agents_md" "gentle-qa:engram-protocol" "Engram section present before switch"
    ...
    assert_file_contains "$agents_md" "gentle-qa:engram-protocol" "Engram section survived persona switch"
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona gentleman 2>&1; then
        ...
        assert_file_contains "$rules" "gentle-qa:sdd-orchestrator" "SDD orchestrator marker present"
        ...
        assert_file_contains "$rules" "gentle-qa:engram-protocol" "Engram protocol marker present"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.gentle-qa/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.gentle-qa/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
