#!/usr/bin/env bash
# lib.sh — shared test helpers for gentle-qa E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/gentle-qa inside
# the Docker container. If not found, fall back to $HOME/gentle-qa or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./gentle-qa in the current repo directory (freshly built local binary)
#   2. ~/gentle-qa (explicit copy in home)
#   3. gentle-qa on PATH (system-installed, e.g. Homebrew)
# This ensures `go build ./cmd/gentle-qa && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    # Prefer the locally built binary (./gentle-qa) produced by `go build ./cmd/gentle-qa`.
    ...
    if [ -x "$repo_root/gentle-qa" ]; then
        echo "$repo_root/gentle-qa"
    elif [ -x "./gentle-qa" ]; then
        echo "./gentle-qa"
    elif [ -x "$HOME/gentle-qa" ]; then
        echo "$HOME/gentle-qa"
    elif command -v gentle-qa >/dev/null 2>&1; then
        echo "gentle-qa"
    else
        echo ""
    fi
...
cleanup_test_env() {
    ...
    rm -rf "$HOME/.gentle-qa" 2>/dev/null || true
...
#
# Full Docker E2E validates gentle-qa's agent/config injection behavior, not the
...
#
# Set GENTLE_QA_E2E_REAL_ENGRAM=1 to opt out and exercise the live download path.
setup_fake_engram_binary() {
    if [ "${GENTLE_QA_E2E_REAL_ENGRAM:-0}" = "1" ]; then
        log_info "Using real Engram binary/download path for E2E"
        return 0
    fi

    local fake_bin_dir="$HOME/.gentle-qa-e2e/bin"
...
# Checks that the gentle-qa section marker appears exactly once (no duplicates).
assert_no_duplicate_section() {
    ...
    local marker="<!-- gentle-qa:${section_id} -->"
    local count
