...
```mermaid
flowchart TD
    START([gentle-qa install]) --> DETECT
    ...
    subgraph PHASE_3["Phase 3: Backup"]
        BACKUP[Backup existing configs<br/>~/.gentle-qa-backup-TIMESTAMP/]
    end
...
**Requirements:**
- R-UPDATE-01: The installer MUST support `gentle-qa update` to check for and install newer versions of itself
- R-UPDATE-02: The installer MUST support `gentle-qa update --skills` to pull latest skill versions for all configured agents
- R-UPDATE-03: The installer MUST support `gentle-qa update --engram` to update Engram to the latest version
...
- R-REL-03: The installer MUST support `gentle-qa repair` to re-run failed steps
...
1. **Naming**: `gentle-qa`, `gentle-qa`, `gai`, or something else? Should it be part of the `Gentleman-Programming` org or standalone?
...
gentle-qa install --preset gentleman --agents claude-code,opencode

# Minimal setup, just Claude Code with basic security
gentle-qa install --preset minimal --agents claude-code

# Team provisioning from shared profile
gentle-qa install --profile ./team-ai-config.yaml

# Update all skills to latest
gentle-qa update --skills

# Update Engram
gentle-qa update --engram

# Backup current configs
gentle-qa backup

# Restore from backup
gentle-qa restore --list
gentle-qa restore --id 2026-02-27-143022
